...
class MyTest(unittest.TestCase):
    '''绑定应用—>设置参数—>配置支付渠道参数—>添加分润模板—>设置收款账号—>创建支付单—>支付成功—>应用方确认收货—>查询结算单详情—>获取交易信息'''
    def setUp(self) -> None:
    ...
    def test_03(self):
        ...
        '''设置应用参数'''
        updateAppInfo_url = js_url + "/appInfo/updateAppInfo"
        ...
        '''配置支付宝渠道参数'''
        updatePayChannel_url = js_url + "/appInfo/updatePayChannel"
        ...
        if 'utMsg' in response_updatePayChannel.text:
            ...
            print('配置支付渠道')
        '''添加分润模板'''
        Template_create_url = js_url + "/royaltyTemplate/create"
        ...
        '''设置用户收款账户'''
        Account_update_url = js_url + "/settlementAccount/update"
        Account_update_data = {
        ...
        '''创建支付单'''
        createPayInfo_url = js_url + "/trade/createPayInfo"
        createPayInfo_data = {
            "amount": 0.01,
            "appId": "app_abcd123",
            "goodsDesc": "1",
        ...
        if 'utMsg' in response_paylist.text:
            ...
            print('支付成功')
        '''查询支付单列表（应用开发者）'''
        paylist_url = js_url + "/settle/pay/list"
        paylist_data = {"appId":"app_abcd123","page":1,"size":1}
        response_paylist = request_host.get_get(paylist_url, headers3, paylist_data)
        if 'utMsg' in response_paylist.text:
            raise ValueError("调用  接口失败,状态码为: %s ,返回信息为：%s" % (response_paylist.status_code),
                             response_paylist.json())
        else:
        ...
        '''应用方确认收货'''
        appConfirm_url = js_url + "/settle/trade/appConfirm"
        ...
        if 'utMsg' in response_appConfirm.text:
            ...
            print('结算成功')
        '''查询结算单详情（应用开发者）'''
        settle_url = js_url + "/settle/settlement/get"
        ...
        if 'utMsg' in response_paylist.text:
            ...
            print('结算详情')
        '''获取应用交易信息'''
        queryAppTradeInfo_url = js_url + "/statistics/queryAppTradeInfo"
