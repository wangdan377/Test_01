...
class MyTest(unittest.TestCase):
    '''绑定应用—>设置参数—>配置支付渠道参数—>添加分润模板—>设置收款账号—>创建支付单—>支付成功—>查询支付单—>创建退款单—>查询退款单详情—>应用方确认收货—>查询结算单详情'''
    def setUp(self) -> None:
    ...
    def test_05(self):
        ...
        createRefundInfo_data ={
            "remark": "tui1",
            "payNo": "413879129855848448",
            "refundAmount": "0.01"}
        response_response_createRefundInfo = request_host.get_get(createRefundInfo_url, headers3, json.dumps(createRefundInfo_data))
        self.assertEqual(200, response_response_createRefundInfo.status_code, "调用【创建退款单】接口失败。状态码错误")
        if 'utMsg' in response_response_createRefundInfo.text:
            raise ValueError("调用  接口失败,状态码为: %s ,返回信息为：%s" % (response_response_createRefundInfo.status_code),
                             response_response_createRefundInfo.json())
        else:
            print('创建退款单')
        '''退款单详情查询（开发者）'''
        refund_url = "https://mobileuat.utcook.com/settle/refund/get"
        refund_data ={"refundId":407777925920792576}
        responserefund = request_host.get_get(refund_url, headers1, refund_data)
        if 'utMsg' in responserefund.text:
            raise ValueError("调用  接口失败,状态码为: %s ,返回信息为：%s" % (responserefund.status_code),
                             responserefund.json())
        else:
            print('退款单详情')
